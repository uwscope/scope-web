...
def _initial_date(
    *,
    ...
    if day_variable_count != 1:
        raise ValueError("Either repeat_day_flags or day_of_week must be provided")
    ...
    if day_of_week:
        ...
        initial_date_earliest = None
        for (day_of_week, flag) in repeat_day_flags.items():
            if flag:
                initial_date_current = start_date + dateutil.relativedelta.relativedelta(
                    weekday=DATEUTIL_WEEKDAYS_MAP[day_of_week]
                )

                if initial_date_earliest is None or initial_date_current < initial_date_earliest:
                    initial_date_earliest = initial_date_current

        return initial_date_earliest
    ...
    *,
    ...
    if repeat_day_flags or day_of_week:
        initial_date: _datetime.date = _initial_date(
            start_date=start_date, repeat_day_flags=repeat_day_flags, day_of_week=day_of_week
        )
    else:
