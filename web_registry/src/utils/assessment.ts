...
export const sortAssessmentContent = (
  a: IAssessmentContent,
  b: IAssessmentContent,
) => {
...
export const getAssessmentScoreFromAssessmentLog = (log: IAssessmentLog) => {
  if (log.totalScore != undefined && log.totalScore >= 0) {
    ...
    return sum(
      Object.keys(log.pointValues).map((k) => log.pointValues[k] || 0),
    );
  }
};

export const getAssessmentScoreFromPointValues = (
...
export const getLatestScore = (
  ...
  const filteredAssessmentLogs = assessmentLogs.filter(
...
export const getAssessmentScoreColorName = (
  assessmentId: string,
  totalScore: number,
) => {
