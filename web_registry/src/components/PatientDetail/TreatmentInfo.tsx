...
export const TreatmentInfo: FunctionComponent = observer(() => {
  ...
  currentPatient.sessions.forEach((s) => {
    Object.keys(s.behavioralStrategyChecklist).forEach((k) => {
      if (
        ...
        behavioralStrategiesUsedFlags[k as BehavioralStrategyChecklistItem] =
          true;
      }
    });
  });
  ...
  return (
    <ActionPanel
      id="treatment"
      title="Ongoing Treatment Information"
      loading={loading}
      error={error}
    >
      <Grid container spacing={2} alignItems="stretch">
        <GridTextField
          ...
          helperText={
            !!latestPhqLog
              ? `Updated: ${format(latestPhqLog.recordedDateTime, "MM/dd/yyyy")}`
              : undefined
          }
        />
        <GridTextField
          ...
          helperText={
            !!latestGadLog
              ? `Updated: ${format(latestGadLog.recordedDateTime, "MM/dd/yyyy")}`
              : undefined
          }
        />
        <GridTextField
          ...
          helperText={
            !!latestSessionDate
              ? `Updated: ${formatDateOnly(latestSessionDate, "MM/dd/yyyy")}`
              : undefined
          }
        />
        <GridTextField
