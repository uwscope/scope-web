...
export const SessionReviewTable: FunctionComponent<ISessionReviewTableProps> =
    observer((props) => {
        ...
        const getLastAvailableAssessment = (
            assessmentLogs: IAssessmentLog[] | undefined,
            date: Date
        ) => {
            var dataPoint = assessmentLogs?.find(
                (p) => compareAsc(p.recordedDate, date) >= 0
            );
