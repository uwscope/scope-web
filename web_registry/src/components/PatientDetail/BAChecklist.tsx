...
export const BAChecklist: FunctionComponent = observer(() => {
  ...
  currentPatient?.sessionsSortedByDate
    ...
    .forEach((s) => {
      Object.keys(s.behavioralActivationChecklist).forEach((key) => {
        if (
          s.behavioralActivationChecklist[
            key as BehavioralActivationChecklistItem
          ]
        ) {
  ...
  const resourcesMap = Object.assign(
    {},
    ...registryresources.map((r) => ({ [r.id]: r.resources })),
  ) as KeyedMap<IResourceItem[]>;
      ...
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>
                {getString("patient_behavioral_checklist_components_header")}
              </TableCell>
              <TableCell>
                {getString("patient_behavioral_checklist_resources_header")}
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
                ...
                >
                  <Fragment>
                    ...
                    {component.completedDate ? (
                      ...
                      <FormHelperText>
                        {getString(
                          "patient_behavioral_checklist_not_completed",
                        )}
                      </FormHelperText>
                    )}
                  </Fragment>
                    ...
                    <List dense disablePadding>
                      {component.resources &&
                        ...
                        component.resources.map((resource, idx) => (
                          ...
                          >
                            <Link
                              href={getResourceLink(resource.filename)}
                              target="_blank"
                            >
                              {resource.name}
                            </Link>
