...
export const AssessmentProgress: FunctionComponent<IAssessmentProgressProps> =
    observer((props) => {
        ...
        const recurrence = assessment.assigned
            ? `${assessment.frequency} on ${
                  assessment.dayOfWeek
              }s, assigned on ${format(assessment.assignedDate, 'MM/dd/yyyy')}`
            : 'Not assigned';
        ...
        return (
                ...
                actionButtons={[
                    {
                        ...
                        text: assessment.assigned
                            ? getString(
                                  'patient_progress_assessment_assigned_button'
                              )
                            : getString(
                                  'patient_progress_assessment_assign_button'
                              ),
                        onClick: assessment.assigned
                            ? undefined
                            : () =>
                                  currentPatient?.assignAssessment(
                            ...
                            ? [
                                  {
                                      icon: <AddIcon />,
                                      text: getString(
                                          'patient_progress_assessment_action_add'
                                      ),
            ...
            >
                    ...
                    {logState.patientSubmitted ? (
                        <DialogActions>
                            <Button onClick={handleClose} color="primary">
                                {getString(
                                    'patient_progress_assessment_dialog_close_button'
                                )}
                            </Button>
                        </DialogActions>
                    ) : (
                        <DialogActions>
                            <Button onClick={handleClose} color="primary">
                                {getString(
                                    'patient_progress_assessment_dialog_cancel_button'
                                )}
                            </Button>
                            <Button
                                onClick={onSaveEditRecord}
                                color="primary"
                                disabled={saveDisabled}
                            >
                                {getString(
                                    'patient_progress_assessment_dialog_save_button'
                                )}
                            </Button>
                        </DialogActions>
                    )}
                ...
                <Dialog
                    open={configureState.openConfigure}
                    onClose={handleClose}
                >
                    <DialogTitle>
                        {getString(
                            'patient_progress_assessment_dialog_configure_title'
                        )}
                    </DialogTitle>
                    <DialogContent dividers>
                        <Grid container spacing={2} alignItems="stretch">
                            ...
                            <GridDropdownField
                                ...
                                sm={12}
                                onChange={(text) =>
                                    onDayOfWeekChange(text as DayOfWeek)
                                }
                            />
                        </Grid>
                    </DialogContent>
