  ...
  return (
    <Grid container spacing={2} alignItems="stretch">
      ...
      <GridMultiOptionsField
        ...
        onChange={(flags) =>
          onReferralStatusFlagsChange(flags as ReferralStatusFlags)
        }
        onOtherChange={(otherFlags) =>
          onOtherReferralFlagsChange(otherFlags as KeyedMap<ReferralStatus>)
        }
      />
      <GridTextField
        sm={12}
...
const ReviewEdit: FunctionComponent<IReviewEditProps> = observer((props) => {
  const {
    review,
    availablePsychiatristNames,
    onReviewValueChange,
    onConsultingPsychiatristChange,
  } = props;
  ...
  return (
    <Grid container spacing={2} alignItems="stretch">
      ...
      <GridTextField
        ...
        onChange={(text) =>
          onReviewValueChange("behavioralStrategyChange", text)
        }
      />
      <GridTextField
        sm={12}
...
export const SessionInfo: FunctionComponent = observer(() => {
  ...
  const onSave = action(async () => {
    ...
    runInAction(() => {
      if (
        !currentPatient.loadClinicalHistoryState.error &&
        !currentPatient.loadCaseReviewsState.error
      ) {
        state.open = false;
      }
    });
  ...
  const availablePsychiatristNames = patientsStore.psychiatrists.map(
    (p) => p.name,
  );
        ...
        content={
          state.entryType == "Session" ? (
            <SessionEdit
              ...
              onReferralStatusFlagsChange={(flags) =>
                onValueChange("referralStatusFlags", flags)
              }
              onOtherReferralFlagsChange={(flags) =>
                onValueChange("otherReferralFlags", flags)
              }
            />
          ) : (
            <ReviewEdit
              ...
              onConsultingPsychiatristChange={
                onReviewConsultingPsychiatristChange
              }
            />
          )
        }
