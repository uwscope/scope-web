...
class RegistryService extends ServiceBase implements IRegistryService {
  ...
  public async getPatients(): Promise<IPatient[]> {
    const response =
      await this.axiosInstance.get<IPatientListResponse>("/patients");
  ...
  public async getProviders(): Promise<IProviderIdentity[]> {
    const response =
      await this.axiosInstance.get<IProviderIdentityListResponse>("/providers");
    return response.data?.providers;
  }
}

export const getRegistryServiceInstance = (baseUrl: string) =>
  new RegistryService(baseUrl) as IRegistryService;
