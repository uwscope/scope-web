...
export class PatientStore implements IPatientStore {
  ...
  public getActivityScheduleById(activityScheduleId: string) {
    return this.activitySchedules.find(
      (as) => as.activityScheduleId == activityScheduleId,
    );
  }

  @action.bound
  public getActivitySchedulesByActivityId(
    activityId: string,
  ): IActivitySchedule[] {
  ...
  public getScheduledAssessmentById(scheduleId: string) {
    return this.loadScheduledAssessmentsQuery.value?.find(
      (t) => t.scheduledAssessmentId == scheduleId,
    );
  }
  ...
  public getTasksByActivityScheduleId(activityScheduleId: string) {
    return this.taskItems.filter(
      (t) => t.activityScheduleId == activityScheduleId,
    );
  }
  ...
  public async load() {
    ...
    await Promise.allSettled([
      ...
      this.loadAndLogQuery<IPatientConfig>(
        this.patientService.getPatientConfig,
        this.loadConfigQuery,
      ),
    ]);
  }
  ...
  public async deleteActivity(activity: IActivity) {
    ...
    const foundIdx = prevActivities.findIndex(
      (a) => a.activityId == activity.activityId,
    );

    console.assert(
      foundIdx >= 0,
      `Activity to delete not found: ${activity.activityId}`,
    );

    if (foundIdx >= 0) {
      const promise = this.patientService
        .deleteActivity(activity)
        .then((_deletedActivity) => {
  ...
  public async updateActivity(activity: IActivity) {
    ...
    console.assert(
      found >= 0,
      `Activity to update not found: ${activity.activityId}`,
    );
  ...
  public async deleteActivitySchedule(activitySchedule: IActivitySchedule) {
    ...
    console.assert(
      foundIdx >= 0,
      `ActivitySchedule to delete not found: ${activitySchedule.activityScheduleId}`,
    );
  ...
  public async updateActivitySchedule(activitySchedule: IActivitySchedule) {
    ...
    console.assert(
      found >= 0,
      `ActivitySchedule to update not found: ${activitySchedule.activityScheduleId}`,
    );
  ...
  public async deleteValue(value: IValue) {
    ...
    console.assert(
      foundIdx >= 0,
      `Value to delete not found: ${value.valueId}`,
    );
  ...
  public async updateValue(value: IValue) {
    ...
    console.assert(
      foundIdx >= 0,
      `Value to update not found: ${value.valueId}`,
    );

    if (foundIdx >= 0) {
      const promise = this.patientService
        .updateValue(value)
        .then((updatedValue: IValue) => {
