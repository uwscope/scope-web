...
export const HomePage: FunctionComponent = observer(() => {
  ...
  const handleOverdueViewClick = action(
    (showPendingOverdueActivities: boolean) => {
      viewState.showPendingOverdueActivities = showPendingOverdueActivities;
    },
  );

  const handleOverdueViewToggle = action(
  ...
  return (
    <MainPage title={getGreeting(new Date())}>
      ...
      <Section title={getString("Home_things_title")}>
        <CompactList>
          ...
          <ListItem
            button
            component={Link}
            to={getFormLink(ParameterValues.form.moodLog)}
          >
            <ListItemAvatar>
              <Avatar
                variant="square"
      ...
      <Section title={getString("Home_plan_title")}>
        {!!rootStore.patientStore.todayItems &&
        rootStore.patientStore.todayItems.length > 0 ? (
          <Fragment>
            {!!rootStore.patientStore.todayItemsCompleted && (
              <Typography variant="body2">
                {getString("Home_plan_done")}
              </Typography>
            )}
            <CompactList>
              {rootStore.patientStore.todayItems.map((item, idx) => (
                <Fragment key={`${item.scheduledActivityId}-${idx}`}>
                  ...
                  {idx < rootStore.patientStore.todayItems.length - 1 && (
                    <Divider variant="middle" />
                  )}
                </Fragment>
              ))}
                ...
                {overdueItems
                  ...
                  .map((item, idx) => (
                    <Fragment key={`${item.scheduledActivityId}-${idx}`}>
                      ...
                      />
                      {idx <
                        overdueItems.filter((i) => !i.completed).length - 1 && (
                        <Divider variant="middle" />
                      )}
                    </Fragment>
                  ))}
