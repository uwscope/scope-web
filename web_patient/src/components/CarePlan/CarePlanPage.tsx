...
import {
  ...
  sortActivitySchedulesByDateAndTime,
} from "shared/sorting";
import {
  formatDateOnly,
  formatDayOfWeekOnly,
...
export const CarePlanPage: FunctionComponent = observer(() => {
  ...
  const selectedTaskItems = taskItems.filter((t) =>
    isSameDay(t.dueDateTime, viewState.selectedDate),
  );
  ...
  const handleTaskClick = action((item: IScheduledActivity) => () => {
    navigate(
      getFormPath(ParameterValues.form.activityLog, {
        [Parameters.activityId]: item.dataSnapshot.activity
  ...
  const handleActivityMoreClick = action(
      ...
      <Stack spacing={1}>
        <HelperText>
          {formatDateOnly(nextTask.dueDateTime, "EEE, MMM d") +
            ", " +
            formatTimeOfDayOnly(
              nextTask.dataSnapshot.activitySchedule.timeOfDay,
            )}
        </HelperText>
          ...
          <HelperText>
            {getRepeatDayText(
              nextTask.dataSnapshot.activitySchedule
                .repeatDayFlags as DayOfWeekFlags,
            )}
          </HelperText>
        )}
      </Stack>
            ...
            {sortedActivities.map((activity) => (
              <Fragment key={activity.activityId}>
                ...
                {(() => {
                  ...
                  sortedActivitySchedules
                    ?.slice()
                    .forEach((activitySchedule) => {
                      const nextTask =
                        patientStore.getNextTaskByActivityScheduleId(
                          activitySchedule.activityScheduleId as string,
                        );
                    ...
                    <ActivityScheduleList>
                      {nextTasks.map((nextTask, idxNextTask) => (
                        ...
                        >
                          ...
                          {idxNextTask < nextTasks.length - 1 && (
    ...
    return (
      <Fragment>
        ...
        >
          ...
          <MenuItem onClick={handleActivityEdit}>
            {getString("menuitem_activity_edit")}
          </MenuItem>
          <MenuItem onClick={handleActivityDelete}>
        ...
        {patientStore.activities.length > 0 ? (
          ...
          <Section title={getString("Careplan_add_activity")}>
            <Typography variant="body2">
              {getString("Careplan_no_activities")}
            </Typography>
          </Section>
        )}
      </Fragment>
    ...
    >
      <Grid container alignItems="center" spacing={1} justifyContent="center">
        <Grid item>
          <ButtonBase onClick={() => handleViewClick(false)}>
            <Typography
              color={viewState.showActivities ? "textSecondary" : "textPrimary"}
            >
        ...
        <Grid item>
          <ButtonBase onClick={() => handleViewClick(true)}>
            <Typography
              color={viewState.showActivities ? "textPrimary" : "textSecondary"}
            >
      ...
      {viewState.showActivities ? (
        ...
        <div>
          ...
          <Section
            title={`Plan for ${format(viewState.selectedDate, "MM/dd/yyyy")}`}
          >
              ...
              <CompactList subheader={<li />}>
                {selectedTaskItems.map((item, idx) => (
                  <Fragment key={item.scheduledActivityId}>
                    ...
                    />
                    {idx < selectedTaskItems.length - 1 && (
                      <Divider variant="middle" />
                    )}
                  </Fragment>
                ))}
