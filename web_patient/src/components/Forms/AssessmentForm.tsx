    ...
    <FormSection
      ...
      content={
        <Stack spacing={2}>
          ...
          <BodyText>
            If you need more help right away, here are some resources to try:
          </BodyText>
          <ListDiv>
            <li>
              <BodyText>
...
const QuestionForm: FunctionComponent<IQuestionFormProps> = (props) => {
  const { instruction, question, questionId, value, options, onValueChange } =
    props;
  return (
    <FormSection
      prompt={instruction}
...
export const AssessmentForm: FunctionComponent<IAssessmentFormProps> = observer(
  () => {
    ...
    if (!assessmentId || !scheduleId) {
      console.error(
        `Scheduled id or assessment id not found in query paramters: ${assessmentId} ${scheduleId}`,
      );
      return null;
    }
    ...
    const scheduledAssessment =
      patientStore.getScheduledAssessmentById(scheduleId);

    if (!scheduledAssessment) {
      console.error(
        `Scheduled assessment not found by schedule id: ${scheduleId}`,
      );
      return null;
    }

    const assessmentContent = rootStore.getAssessmentContent(
      scheduledAssessment?.assessmentId,
    );
    ...
    const dataState = useLocalObservable<{
      pointValues: ObservableMap<string, number>;
      comment: string;
    }>(() => ({
    ...
    const getAssessmentPages = () => {
      ...
      const total = getAssessmentScore(
        Object.fromEntries(dataState.pointValues.entries()),
      );
      const assessment =
      ...
      const scorePage = {
        content: (
          <Stack spacing={4}>
            ...
            />
            {assessmentId == "phq-9" &&
              !!dataState.pointValues.get("Suicide") && <CrisisContent />}
              ...
              content={
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell>
                        {getString("Form_assessment_score_column_name")}
                      </TableCell>
                      <TableCell>
