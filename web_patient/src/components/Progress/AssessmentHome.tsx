...
export const AssessmentHome: FunctionComponent<{ assessmentType: string }> =
  observer((props) => {
    ...
    const title =
      assessmentType == "phq-9"
        ? "Progress_phq_assessment_title"
        : "Progress_gad_assessment_title";
    const detail_title =
      assessmentType == "phq-9"
        ? "Progress_phq_assessment_detail_title"
        : "Progress_gad_assessment_detail_title";
    const assessmentContent = rootStore.getAssessmentContent(assessmentType);

    const sortedLogs = patientStore.assessmentLogs
                  ...
                  content={
                    <Table size="small" aria-label="a dense table">
                      <TableBody>
                        ...
                        {viewState.selectedLog &&
                          viewState.selectedLog.pointValues &&
                          Object.keys(viewState.selectedLog.pointValues).map(
                            (key) => (
                              <TableRow key={key}>
                                <TableCell component="th" scope="row">
                                  {key}
                                ...
                                <WordBreakTableCell>
                                  {getValueString(
                                    viewState.selectedLog?.pointValues[key],
                                  )}
                                </WordBreakTableCell>
                              </TableRow>
                            ),
